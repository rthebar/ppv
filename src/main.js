...
function renderSports(categories) {
  ...
  sorted.forEach((cat) => {
    ...
    li.innerHTML = `
      ...
      <span class="sport-name">${cat.category}</span>
    `;
...
function renderStreams(streams) {
  ...
  if (state.searchQuery) {
    filtered = filtered.filter((s) => s.name.toLowerCase().includes(state.searchQuery) || (s.sourceTag || '').toLowerCase().includes(state.searchQuery));
  }
...
function createStreamCard(stream, index) {
  ...
  card.innerHTML = `
    ${posterUrl ? `<img class="card-poster" src="${posterUrl}" alt="${stream.name}" loading="lazy" onerror="this.style.display='none'" />` : ''}
    <div class="card-body">
      <div class="match-card-header">
        <div class="match-badges">
          ...
          <span class="badge badge-sport">${stream.categoryName}</span>
          ${stream.sourceTag ? `<span class="badge badge-tag">${stream.sourceTag}</span>` : ''}
        </div>
        <div class="match-header-right">
          ${viewerText ? `<span class="viewers-badge">${viewerText}</span>` : ''}
          <span class="match-time">${status === 'live' ? 'LIVE' : stream.date ? formatMatchTime(stream.date) : ''}</span>
        </div>
      </div>
      <div class="event-title">${stream.name}</div>
      <div class="match-card-footer">
        <span class="match-date">
          <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
          ${stream.date ? formatMatchTime(stream.date) : ''}
        </span>
